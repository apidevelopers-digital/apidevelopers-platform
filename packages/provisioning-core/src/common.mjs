export class ProvisioningDomainError extends Error {
  constructor(code, message, details = {}) {
    super(message);
    this.name = "ProvisioningDomainError";
    this.code = code;
...
export function requireText(value, name) {
  ...
  if (!result) {
    throw new ProvisioningDomainError("invalid_argument", `${name} is required`);
  }
  return result;
}
...
export function requireIso(value, name) {
  ...
  if (Number.isNaN(Date.parse(result))) {
    throw new ProvisioningDomainError("invalid_argument", `${name} must be an ISO date`);
  }
  return result;
}
...
const SENSITIVE_KEY =
  /(authorization|bearer|password|secret|token|api.?key|private.?key|card|cvv|cvc|pan)/i;
  ...
  for (const [key, nested] of Object.entries(value)) {
    if (SENSITIVE_KEY.test(key)) {
      throw new ProvisioningDomainError(
        ...
        `${path}.${key} cannot be stored by provisioning-core`,
      );
    }
