...
function requireSha256(value, field) {
  ...
  if (!/^sha256:[0-9a-f]{64}$/i.test(normalized)) {
    fail("invalid_generation_digest", `${field} must be sha256:<64 hex>`);
  }
...
function requireCommit(value) {
  ...
  if (!/^[0-9a-f]{40}$/i.test(normalized)) {
    fail("invalid_generation_commit", "codeCommit must be a 40-hex git commit");
  }
  return normalized.toLowerCase();
}
...
function requirePositiveInt(value, field) {
  if (!Number.isInteger(value) || value < 1 || value > 10000000) {
    fail("invalid_generation_count", `${field} must be a positive integer`);
  }
  return value;
}
...
function assertFixedPolicy(receipt) {
  if (
    receipt.evaluationOnly !== true ||
    receipt.trainingUsed !== false ||
    receipt.rawBiometricsRetained !== false ||
    receipt.rawEmbeddingsRetained !== false
  ) {
    fail("generation_receipt_policy_mismatch", "score generation receipt policy mismatch");
  }

  if (
    receipt.provenanceClass !== TRUST_FACE_CONSENTED_SCORE_GENERATION_RECEIPT_V1.provenanceClass ||
    receipt.originAttested !== false ||
    receipt.realMetricsReady !== false ||
    receipt.productionReady !== false ||
    receipt.biometricClaimReady !== false
  ) {
    fail(
      "generation_receipt_claim_state_mismatch",
      "score generation receipt provenance/claim state mismatch",
    );
  }
}
...
function buildBody({
  ...
  evaluationOnly,
  trainingUsed,
  rawBiometricsRetained,
  rawEmbeddingsRetained,
  provenanceClass,
  originAttested,
  realMetricsReady,
  productionReady,
...
export function createConsentedScoreGenerationReceipt({
  ...
  if (evaluationOnly !== true) {
    fail("generation_evaluation_only_required", "evaluationOnly must be true");
  }
  if (trainingUsed !== false) {
    fail("generation_training_forbidden", "trainingUsed must be false");
  }
  if (rawBiometricsRetained !== false) {
    fail("generation_raw_biometrics_retention_forbidden", "rawBiometricsRetained must be false");
  }
  if (rawEmbeddingsRetained !== false) {
    fail("generation_raw_embeddings_retention_forbidden", "rawEmbeddingsRetained must be false");
  }

  const started = parseIso(startedAt, "startedAt");
  const completed = parseIso(completedAt, "completedAt");
  if (completed.ms <= started.ms) {
    fail("generation_invalid_window", "completedAt must be after startedAt");
  }
  ...
  const body = buildBody({
    ...
    rawBiometricsRetained: false,
    rawEmbeddingsRetained: false,
    provenanceClass: TRUST_FACE_CONSENTED_SCORE_GENERATION_RECEIPT_V1.provenanceClass,
    originAttested: false,
    realMetricsReady: false,
    productionReady: false,
    biometricClaimReady: false,
...
export function assertConsentedScoreGenerationReceipt({
  ...
  if (!receipt || typeof receipt !== "object" || Array.isArray(receipt)) {
    fail("generation_receipt_required", "score generation receipt is required");
  }
  if (receipt.version !== TRUST_FACE_CONSENTED_SCORE_GENERATION_RECEIPT_V1.version) {
    fail("generation_receipt_version_mismatch", "unsupported score generation receipt version");
  }
  if (
    receipt.purpose !== TRUST_FACE_CONSENTED_SCORE_GENERATION_RECEIPT_V1.purpose ||
    receipt.executionMode !== TRUST_FACE_CONSENTED_SCORE_GENERATION_RECEIPT_V1.executionMode
  ) {
    fail(
      "generation_receipt_purpose_mismatch",
      "score generation receipt purpose/executionMode mismatch",
    );
  }

  assertFixedPolicy(receipt);
  ...
  if (completed.ms <= started.ms) {
    fail("generation_invalid_window", "receipt completedAt must be after startedAt");
  }
  if (now !== null && parseIso(now, "now").ms < completed.ms) {
    fail("generation_receipt_from_future", "score generation receipt completedAt is after now");
  }
  ...
  const body = buildBody({
    ...
    scoreSourceManifestDigest: requireSha256(
      receipt.scoreSourceManifestDigest,
      "receipt.scoreSourceManifestDigest",
    ),
    checkpointManifestDigest: requireSha256(
      receipt.checkpointManifestDigest,
      "receipt.checkpointManifestDigest",
    ),
    ...
    startedAt: started.iso,
    completedAt: completed.iso,
    evaluationOnly: receipt.evaluationOnly,
    trainingUsed: receipt.trainingUsed,
    rawBiometricsRetained: receipt.rawBiometricsRetained,
    rawEmbeddingsRetained: receipt.rawEmbeddingsRetained,
    provenanceClass: receipt.provenanceClass,
    originAttested: receipt.originAttested,
    realMetricsReady: receipt.realMetricsReady,
    productionReady: receipt.productionReady,
    biometricClaimReady: receipt.biometricClaimReady,
  });

  const expectedReceiptDigest = sha256(body);
  if (receipt.generationReceiptDigest !== expectedReceiptDigest) {
    fail(
      "generation_receipt_digest_mismatch",
      "score generation receipt digest mismatch",
    );
  }
  ...
  for (const [field, value] of Object.entries(expected)) {
    if (body[field] !== value) {
      fail(
        `generation_${field}_mismatch`,
        `score generation receipt ${field} mismatch`,
      );
    }
  }
