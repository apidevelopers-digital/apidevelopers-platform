...
function repo(initial = []) {
  ...
  return {
    ...
    async getById(id) {
      return map.has(id) ? structuredClone(map.get(id)) : null;
    },
    async list() {
      return [...map.values()].map((record) => structuredClone(record));
    },
...
test("raw payloads, ciphertext and key material are rejected", () => {
  for (const field of [
    "image", "video", "embedding", "vector", "template",
    "plaintext", "ciphertext", "wrappedDataKey", "dataKey", "keyMaterial",
  ]) {
    assert.throws(
      () => createTemplateVaultEnvelopeLabRecord({ ...input(), [field]: "forbidden" }),
      (error) =>
        error instanceof TrustFaceTemplateVaultEnvelopeLabV1Error &&
        error.code === "template_vault_payload_forbidden",
    );
  }
});
...
test("assertion validates record and rejects future createdAt", () => {
  ...
  const checked = assertTemplateVaultEnvelopeLabRecord({
    record,
    enrollmentManifest: manifest,
    now: "2026-09-01T03:40:00Z",
  });
  ...
  assert.throws(
    () => assertTemplateVaultEnvelopeLabRecord({
      record,
      enrollmentManifest: manifest,
      now: "2026-09-01T03:29:59Z",
    }),
    (error) => error?.code === "template_vault_record_from_future",
  );
});

test("tampering with policy or record digest is rejected", () => {
  const record = createTemplateVaultEnvelopeLabRecord(input());
  assert.throws(
    () => assertTemplateVaultEnvelopeLabRecord({
      record: { ...record, encryptionPerformed: true },
      enrollmentManifest: manifest,
      now: "2026-09-01T03:40:00Z",
    }),
    (error) => error?.code === "template_vault_policy_mismatch",
  );
  assert.throws(
    () => assertTemplateVaultEnvelopeLabRecord({
      record: { ...record, recordDigest: d("9") },
      enrollmentManifest: manifest,
      now: "2026-09-01T03:40:00Z",
    }),
    (error) => error?.code === "template_vault_record_digest_mismatch",
  );
});
...
test("tampering with manifest binding is rejected", () => {
  ...
  assert.throws(
    () => assertTemplateVaultEnvelopeLabRecord({
      record: { ...record, enrollmentManifestDigest: d("8") },
      enrollmentManifest: manifest,
      now: "2026-09-01T03:40:00Z",
    }),
    (error) => error?.code === "template_vault_enrollmentManifestDigest_mismatch",
  );
});
...
test("persistence registers reads and lists verified records without mutation APIs", async () => {
  ...
  assert.deepEqual(
    await persistence.get(manifest.enrollmentId, { now: "2026-09-01T03:40:00Z" }),
    created,
  );
  assert.deepEqual(
    await persistence.list({ now: "2026-09-01T03:40:00Z" }),
    [created],
  );
...
test("persistence fails closed for tampered and orphan records", async () => {
  ...
  repository.unsafeMutate(manifest.enrollmentId, (record) => {
    record.keyProviderReady = true;
  });
  await assert.rejects(
  ...
  await assert.rejects(
    ...
    (error) => error?.code === "template_vault_orphan_record",
  );
});
