...
- simulation-only governed template vault flow that composes enrollment persistence, metadata-only vault receipt persistence, explicit metadata-read authorization and governed revocation into one end-to-end lifecycle facade; it exposes lab-only authorization builders, preserves revocation denial before access authorization, emits lifecycle snapshots without payloads, and rejects raw biometric/template/ciphertext/key/KMS/secret payloads at the composed boundary;
- simulation-only detached Ed25519 authorization-proof verifier for vault metadata reads, using a trusted public key injected at runtime and signing only the canonical `authorizationDigest`; the composed facade requires the existing governed authorization/access checks before proof verification, rejects untrusted keys/private keys/tampered digests/invalid signatures, and does not integrate an external issuer or production trust registry;
- simulation-only append-only trusted-public-key registry for vault access proofs, with Ed25519 public-key registration/fingerprinting, validity windows, revocation lifecycle and registry-backed proof access; no private keys, external issuer/revocation authority, production trust registry, KMS or real-vault trust;
- simulation-only signed governed template vault flow that composes the governed enrollment/receipt/revocation lifecycle with registry-backed detached Ed25519 proof verification; it accepts externally produced proofs only, exposes no signing/private-key path, and preserves both enrollment-revocation and trusted-key-revocation denial;
...
The template vault access authorization contract is likewise simulation/lab-only and metadata-only. An active enrollment/receipt is not sufficient by itself: `getAuthorizedReceipt` additionally requires a full authorization object with exact receipt/governance bindings, an allowed purpose and an active time window.
`digestOnlyAccessAccepted=false`, `broadListingAuthorized=false`, `realVaultAccessAuthorized=false` and `productionReady=false` remain explicit; this contract does not authorize access to any biometric template, ciphertext, key/KMS/secret material, decryption capability or real vault.
The governed template vault flow is an integration/lifecycle harness, not a production vault. `endToEndLifecycleComposed=true` means only that the existing lab contracts are exercised together from enrollment through receipt access and revocation. `authorizationIssuerIntegrated=false`, `cryptographicAuthorizationProofVerified=false`, `realVaultAccessAuthorized=false`, `realVaultRevocationEnforced=false` and `productionReady=false` remain explicit.
The detached Ed25519 access-proof verifier closes only a laboratory integrity gap: `cryptographicAuthorizationProofVerifiedInLab=true` means a signature over the already-derived `authorizationDigest` was verified against an explicitly trusted runtime public key. `externalAuthorizationIssuerIntegrated=false`, `productionTrustRegistryIntegrated=false`, `productionCryptographicAuthorizationProofVerified=false`, `realVaultAccessAuthorized=false` and `productionReady=false` remain explicit.
The lab trust registry makes `labTrustRegistryIntegrated=true` only for the in-package simulation contract. `productionTrustRegistryIntegrated=false`, `productionKeyManagementIntegrated=false`, `externalAuthorizationIssuerIntegrated=false`, `externalRevocationAuthorityIntegrated=false`, `productionCryptographicAuthorizationProofVerified=false`, `realVaultAccessAuthorized=false` and `productionReady=false` remain explicit.
Key revocation is logical append-only metadata and does not prove physical key deletion.
The signed governed template vault flow is also only a composition harness. `signedAccessLifecycleComposed=true`, `labTrustRegistryIntegrated=true` and `cryptographicAuthorizationProofVerifiedInLab=true` mean the existing lab lifecycle and proof contracts are exercised together; `signingPerformed=false`, `privateKeyAccepted=false`, `externalAuthorizationIssuerIntegrated=false`, `productionTrustRegistryIntegrated=false`, `productionKeyManagementIntegrated=false`, `productionCryptographicAuthorizationProofVerified=false`, `realVaultAccessAuthorized=false` and `productionReady=false` remain explicit.
