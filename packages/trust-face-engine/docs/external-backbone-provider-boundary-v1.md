...
Raw image/video/pixel/binary payloads are not accepted here. Sensitive provider configuration such as private keys, KMS material, secrets, tokens or inline payloads is rejected before provider invocation. A successful call means only that the injected provider returned a finite 512-dimensional embedding through the authorized candidate boundary.
