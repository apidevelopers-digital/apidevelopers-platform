...
export class DecisionEngine {
  ...
  decide({tenantId,cycleId,planningReport}={}, {evidence=[],reviews=[],requestedBy="system"}={}){
    str(tenantId,"tenantId"); str(cycleId,"cycleId"); str(requestedBy,"requestedBy");
    ...
    const before=clone(planningReport), ev=usableEvidence(evidence), rv=approvedReviews(reviews);
    const candidates=(planningReport.proposals??[]).map((p)=>{
      ...
      return {...clone(p), missingEvidence:[...missingEvidence].sort(), missingReviews:[...missingReviews].sort(), decisionState, eligible:decisionState==="ready-for-human-decision"};
    }).sort((a,b)=>(PRIORITY.get(a.priority)??99)-(PRIORITY.get(b.priority)??99) || a.proposalId.localeCompare(b.proposalId));
    ...
    const report=freeze({
      ...
      generatedAt, requestedBy, tenantId, cycleId,
      sourcePlanningId:planningReport.planningId,
      selectedProposalId:selected?.proposalId??null,
      decisionState:selected?"ready-for-human-decision":(candidates.some(c=>c.decisionState==="needs-evidence")?"needs-evidence":candidates.some(c=>c.decisionState==="needs-review")?"needs-review":"blocked"),
      ...
      constraints:{automaticDecisionAllowed:false, automaticApprovalAllowed:false, automaticExecutionAllowed:false, tenantIsolationRequired:true, traceabilityRequired:true}
    });
