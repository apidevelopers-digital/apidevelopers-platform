...
export function createAccessGrantId(tenantSlug, workspaceSlug, productId, principalId) {
  return createCanonicalId({
    family: "component",
    segments: ["access", tenantSlug, workspaceSlug, productId, principalId],
  });
}
...
export function createAccessGrant(input = {}) {
  ...
  if (!STATUSES.includes(grant.status)) {
    throw new RangeError("invalid access grant status");
  }
  if (grant.status === "active" && !grant.activatedAt) {
    throw new Error("active access grant requires activatedAt");
  }
...
export function createOnboardingState(input = {}) {
  const state = {
    ...
    productId: requireText(input.productId, "productId"),
    status: input.status ?? "pending",
    requiredSteps: uniqueTexts(input.requiredSteps, "requiredStep"),
    completedSteps: uniqueTexts(input.completedSteps, "completedStep"),
  ...
  if (!ONBOARDING_STATUSES.includes(state.status)) {
    throw new RangeError("invalid onboarding status");
  }

  return Object.freeze(state);
}

export function assertAutomatedAccessReadiness({
  subscription,
  entitlement,
  provisioningJob,
  grant,
  ...
  if (!subscription || subscription.status !== "active") {
    throw new Error("access requires active subscription");
  }
  if (!entitlement || entitlement.status !== "active") {
    throw new Error("access requires active entitlement");
  }
  if (!provisioningJob || provisioningJob.status !== "succeeded") {
    throw new Error("access requires succeeded provisioning");
  }
  if (!grant) {
    throw new Error("access grant required");
  }
  ...
  for (const [field, expected] of subscriptionBindings) {
    if (grant[field] !== expected) {
      throw new Error("access binding mismatch");
    }
  }
...
export {
  STATUSES as accessGrantStatuses,
  ONBOARDING_STATUSES as onboardingStatuses,
};
