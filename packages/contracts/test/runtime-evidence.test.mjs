...
function makeRuntimeReport(overrides = {}) {
  return {
...
test("creates an immutable runtime -> evidence handoff", () => {
  ...
  assert.ok(Object.isFrozen(handoff.payload.runtimeReport));
});
...
test("rejects a runtime report from another tenant", () => {
  assert.throws(
    () => createRuntimeEvidenceHandoff({
      ...
      cycleId: "cycle.0001",
      tenantContext: makeTenantContext(),
      runtimeReport: makeRuntimeReport({ tenantId: "tenant_other_0001" }),
      createdAt: "2026-07-19T06:02:00.000Z",
    }),
    /tenantId mismatch/,
  );
...
test("validates an immutable runtime evidence record", () => {
  ...
  const record = {
    ...
    tenantId: runtimeReport.tenantId,
    type: "runtime-report",
    source: {
