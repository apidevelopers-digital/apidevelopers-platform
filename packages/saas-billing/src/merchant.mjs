...
export function createFinancialControl({
  legalEntities = [],
  merchantAccounts = [],
  bindings = [],
} = {}) {
  ...
  const b = bindings.map((x) => {
    ...
    const businessUnitId = text(x.businessUnitId, "businessUnitId");
    const brandId = text(x.brandId, "brandId");
  ...
  return Object.freeze({
    resolve({ productId, countryCode, currency, environment = "test" }) {
      const hits = b.filter(x =>
        x.status === "active" &&
        x.productId === productId &&
        x.countries.includes(countryCode) &&
        x.currencies.includes(currency) &&
        m.get(x.merchantAccountId)?.status === "active" &&
        m.get(x.merchantAccountId)?.environment === environment
      );
      if (hits.length !== 1) throw new Error(hits.length ? "ambiguous product seller binding" : "no active product seller binding");
      const x = hits[0];
      return Object.freeze({
        binding: x,
        legalEntity: e.get(x.legalEntityId),
        merchantAccount: m.get(x.merchantAccountId),
      });
    },
  });
}
...
export function createFiscalDocumentRequest(x = {}) {
  ...
  return Object.freeze({
    ...
    legalEntityId: text(x.legalEntityId, "legalEntityId"),
    businessUnitId: text(x.businessUnitId, "businessUnitId"),
    productId: text(x.productId, "productId"),
    fiscalProfileId: text(x.fiscalProfileId, "fiscalProfileId"),
