const obj=(v,n)=>{if(!v||typeof v!=="object")throw new TypeError(`${n} must be an object`)};
const text=(v,n)=>{if(typeof v!=="string"||!v.trim())throw new TypeError(`${n} must be a non-empty string`);return v.trim()};
const major=n=>Number.parseFloat((n/100).toFixed(2));
const recurring=i=>i==="month"?{frequency:1,frequency_type:"months"}:i==="year"?{frequency:12,frequency_type:"months"}:(()=>{throw new TypeError(`unsupported Mercado Pago interval: ${i}`)})();
const eventType=s=>s==="approved"?"payment.succeeded":s==="rejected"?"payment.failed":["cancelled","cancelled_by_user"].includes(s)?"subscription.cancelled":"checkout.completed";
...
export function createMercadoPagoSubscriptionProvider({client,mode="test"}={}){
  ...
  return Object.freeze({
    name:"mercadopago",
    mode,
    async createCheckoutSession({checkoutIntentId,tenantId,workspaceId,subscriptionId,price,successUrl}){
      const plan=await client.createSubscriptionPlan({
        reason:`${price.productId} ${price.planId}`,
        auto_recurring:{...recurring(price.interval),transaction_amount:major(price.amountMinor),currency_id:price.currency},
        back_url:successUrl,
