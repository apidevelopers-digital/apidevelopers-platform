...
function parseRegex(specification) {
  if (typeof specification === "string") {
    return new RegExp(specification, "u");
  }
...
function ensureIo(io) {
  if (typeof io?.readText !== "function") {
    throw new TypeError("Built-in adapters require readText().");
  }
  if (typeof io?.exists !== "function") {
    throw new TypeError("Built-in adapters require exists().");
  }
}
  ...
  return Object.freeze({
    async "required-path"({ rule, targets }) {
      ...
      for (const expectedPath of exactPaths.map(normalizeRepositoryPath).sort()) {
        if (!(await io.exists(expectedPath))) {
      ...
      if (forEach) {
        ...
        const anchors = [...new Set(targets.map(normalizeRepositoryPath))]
          .filter((target) => selector.test(target))
          .sort();
        ...
        for (const anchor of anchors) {
          ...
          for (const relativePath of [...relativePaths].sort()) {
            ...
            if (!(await io.exists(expectedPath))) {
              findings.push(createFinding(rule, {
                path: expectedPath,
                observed: { exists: false, anchor },
                expected: { exists: true, relativeTo: anchor },
              }));
            }
          }
        }
    ...
    async "required-field"({ rule, targets }) {
      ...
      const expectedPattern = rule?.parameters?.pattern
        ? parseRegex(rule.parameters.pattern)
        : null;
        ...
        if (!observed.found || !matchesValue || !matchesPattern) {
          findings.push(createFinding(rule, {
            ...
            observed: {
              ...
              value: observed.found ? observed.value : null,
            },
            expected: {
              pointer,
              ...(expectedValue === undefined ? {} : { equals: expectedValue }),
              ...(rule?.parameters?.pattern ? { pattern: rule.parameters.pattern } : {}),
            },
          }));
        }
      }
    ...
    async "allowed-value"({ rule, targets }) {
      ...
      for (const target of selectTargets(rule, targets)) {
        ...
        if (!observed.found || !values.some((value) => Object.is(value, observed.value))) {
          findings.push(createFinding(rule, {
            path: target,
