...
function required(value, name) {
  ...
  if (!result) throw new ProjectDomainError("invalid_argument", `${name} is required`);
  return result;
}
...
export function createMemoryProjectRepository({ initialProjects = [] } = {}) {
  ...
  function store(input, replace = false) {
    ...
    if (existing) {
      keyToId.delete(projectKey(existing.tenantId, existing.slug));
    }
...
export function createProjectService({
  ...
  return Object.freeze({
    ...
    createProject({ tenantId, name, slug, metadata = {} }) {
      assertTenantOperational(required(tenantId, "tenantId"));
      const createdAt = now();
