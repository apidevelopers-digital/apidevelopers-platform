...
test("normalizes slugs and returns immutable records", () => {
  assert.equal(normalizeProjectSlug(" Projeto Árvore / API "), "projeto-arvore-api");
  const project = record({ metadata: { region: "br" } });
  assert.equal(project.slug, "projeto-arvore");
  assert.throws(() => { project.metadata.region = "us"; }, TypeError);
});
...
test("service validates tenant and emits lifecycle events", () => {
  ...
  assert.equal(service.activateProject("project-1").events[0].type, "project.activated");
  ...
  assert.equal(service.restoreProject("project-1").events[0].type, "project.reactivated");
  assert.equal(service.archiveProject("project-1").project.status, "archived");
  assert.equal(service.restoreProject("project-1").events[0].type, "project.restored");
...
test("lists projects by tenant and status deterministically", () => {
  ...
  assert.deepEqual(
    repository.listByTenant("tenant-1", { status: "active" }).map(({ id }) => id),
    ["project-2"],
  );
});
