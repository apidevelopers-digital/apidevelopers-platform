  ...
  analyze(snapshot, { scope = "platform", requestedBy = "system" } = {}) {
    ...
    for (const node of nodes) {
      ...
      if (prefix === "capability") {
        const hasAsset = relations.some((relation) => prefixOf(otherEnd(relation, node.id)) === "asset");
        if (!hasAsset) {
          findings.push(
            finding(
              "REF-002",
      ...
      if (prefix === "asset") {
        const hasEvidence = relations.some((relation) => prefixOf(otherEnd(relation, node.id)) === "evidence");
        if (!hasEvidence) {
          findings.push(
            finding(
              "REF-003",
      ...
      if (["org", "organization"].includes(prefix) && (!node.status || node.status === "active")) {
        const hasSolution = relations.some((relation) => prefixOf(otherEnd(relation, node.id)) === "solution");
        if (!hasSolution) {
          findings.push(
            finding(
              "REF-004",
    ...
    return {
      reflectionId: `reflection.${generatedAt.replace(/[-:.TZ]/g, "").toLowerCase()}`,
      generatedAt,
      requestedBy,
      scope,
      mode: "advisory",
