...
export function requireText(value, name) {
  ...
  if (!result) throw new GatewayDomainError("invalid_argument", `${name} is required`);
  return result;
}
...
export function requireIso(value, name) {
  ...
  if (Number.isNaN(Date.parse(result))) {
    throw new GatewayDomainError("invalid_argument", `${name} must be an ISO date`);
  }
  return result;
}
...
export function requirePositiveInteger(value, name) {
  if (!Number.isSafeInteger(value) || value < 1) {
    throw new GatewayDomainError("invalid_argument", `${name} must be a positive safe integer`);
  }
  return value;
}
  ...
  for (const [key, nested] of Object.entries(value)) {
    if (SENSITIVE_KEY.test(key)) {
      throw new GatewayDomainError(
        ...
        `${path}.${key} cannot be stored by gateway-core`,
      );
    }
