...
export function requireText(value, name) {
  ...
  if (!result) {
    throw new ActivationDomainError("invalid_argument", `${name} is required`);
  }
  return result;
}
...
export function requireIso(value, name) {
  ...
  if (Number.isNaN(Date.parse(result))) {
    throw new ActivationDomainError("invalid_argument", `${name} must be an ISO date`);
  }
  return result;
}
  ...
  for (const [key, nested] of Object.entries(value)) {
    if (SENSITIVE_KEY.test(key)) {
      throw new ActivationDomainError(
        ...
        `${path}.${key} cannot be stored by activation-core`,
      );
    }
