...
(
  cd "$MCP_WORKDIR"
  npm init -y >/dev/null
  npm install --no-audit --no-fund hostinger-api-mcp@1.26.0 @modelcontextprotocol/sdk@1.10.0
)
export HOSTINGER_MCP_BIN="$MCP_WORKDIR/node_modules/.bin/hostinger-hosting-mcp"
node "$MCP_WORKDIR/client.mjs" "$CARRIER"

export HOSTINGER_DOMAIN NODE_ARCHIVE_NAME
node - <<'NODE'
...
node apps/site-factory/src/run-hostinger-node-build-api-only.mjs \
  ...
  --archive-representation-verified true \
  --node-version 22 \
  --app-type express \
  --output-directory "" \
  --build-script "" \
  ...
  --package-manager npm \
  --expected-text "uni.co" \
  --evidence "$EVIDENCE"

node - <<'NODE'
const url = "https://unico-preview.apidevelopers.digital/health";
let last = null;
for (let i=0;i<24;i++) {
  try {
    const r=await fetch(url,{redirect:"follow"});
    ...
    if (r.ok) {
      ...
      if (b?.ok===true && b?.product==="uni-co-web") {
        console.log("preview_health_ok=true");
        process.exit(0);
      }
    }
