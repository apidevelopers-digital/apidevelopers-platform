...
export function createHostingerHostingPreflightReport({
  orderReference,
  datacentersPayload,
  ...
  if (datacenters.length === 0) {
    throw new Error("hostinger_hosting_datacenters_unavailable");
  }
  ...
  const report = {
    ...
    endpoints: {
      datacenters: "/api/hosting/v1/datacenters?order_id={order_id}",
      createWebsite: "/api/hosting/v1/websites",
      nodeBuildFromArchive:
...
export async function runHostingerHostingPreflight({
  token,
  orderId,
  fetchImpl = fetch,
  ...
  const url = new URL("/api/hosting/v1/datacenters", baseUrl);
  url.searchParams.set("order_id", order);
  ...
  if (!response.ok) {
    throw new Error(
      `hostinger_hosting_preflight_failed:${response.status}:${response.statusText}`,
    );
  }
...
// Compatibility aliases retained while file names and package scripts are migrated.
export const createHostingerAgencyPreflightReport =
