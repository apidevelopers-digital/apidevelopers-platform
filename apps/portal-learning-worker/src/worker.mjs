...
export function resolveConfig(env = process.env) {
  return {
    ...
    auditPath: env.PORTAL_LEARNING_AUDIT_PATH ?? "./var/learning-audit.json",
...
export async function startWorker(env = process.env) {
  ...
  const timer = setInterval(() => {
    runCycle(config).catch((error) => {
      console.error(JSON.stringify({ event: "portal_learning_publish_failed", message: error.message }));
    });
  }, config.intervalMs);
  timer.unref();
}
