import assert from "node:assert/strict";
import { mkdtemp, rm } from "node:fs/promises";
import { tmpdir } from "node:os";
import { join } from "node:path";
import test from "node:test";

import {
  createUseCaseRegisteredOperationalGateway,
} from "../src/operational-use-case-registry-composition.mjs";

function principal(tenantId, id, kind, scopes) {
  return { id, tenantId, kind, scopes };
}

function request(apiKey, method, url, body, headers = {}) {
  return {
    method,
    url,
    headers: {
      "x-api-key": apiKey,
      "content-type": "application/json",
      "x-correlation-id": "corr_http",
      ...headers,
    },
    ...(body === undefined ? {} : { body: JSON.stringify(body) }),
  };
}

const confirmation = {
  "x-operation-confirmation": "IGOR_APROVA_EXECUCAO",
};

test("exposes a governed use case registry without an execution route", async () => {
  const directory = await mkdtemp(
    join(tmpdir(), "use-case-registry-http-"),
  );
  const stateFilePath = join(directory, "state.json");

  try {
    const operator = createUseCaseRegisteredOperationalGateway({
      stateFilePath,
      adminKey: "operator-secret",
      adminPrincipal: principal(
        "tenant_001",
        "operator_001",
        "human",
        [
          "model:read",
          "model:write",
          "usecase:read",
          "usecase:write",
        ],
      ),
    });

    const modelBody = {
      modelId: "model_support_v1",
      provider: "provider_a",
      model: "safe-model",
      version: "2026-07-01",
      purpose: "customer_support",
      dataPolicyId: "policy_support_v1",
      allowedLocales: ["pt-BR"],
      correlationId: "corr_model_register",
    };

    const modelRegistered = await operator.app.handleRequest(
      request(
        "operator-secret",
        "POST",
        "/v1/global-trust/models",
        modelBody,
        confirmation,
      ),
    );
    assert.equal(modelRegistered.status, 201);

    const modelApproved = await operator.app.handleRequest(
      request(
        "operator-secret",
        "POST",
        "/v1/global-trust/models/model_support_v1/status",
        {
          status: "approved",
          reasonCode: "evaluation_passed",
          correlationId: "corr_model_approve",
        },
        confirmation,
      ),
    );
    assert.equal(modelApproved.status, 200);

    const useCaseBody = {
      useCaseId: "use_case_support_v1",
      ownerId: "team_support",
      purpose: "customer_support",
      dataPolicyId: "policy_support_v1",
      riskLevel: "moderate",
      allowedModelIds: ["model_support_v1"],
      allowedToolIds: ["catalog.read"],
      allowedLocales: ["pt-BR"],
      humanApprovalRequired: true,
      reasonCode: "initial_registration",
      correlationId: "corr_use_case_register",
    };

    const missingConfirmation = await operator.app.handleRequest(
      request(
        "operator-secret",
        "POST",
        "/v1/global-trust/use-cases",
        useCaseBody,
      ),
    );
    assert.equal(missingConfirmation.status, 428);

    const injectedTenant = await operator.app.handleRequest(
      request(
        "operator-secret",
        "POST",
        "/v1/global-trust/use-cases",
        { ...useCaseBody, tenantId: "tenant_other" },
        confirmation,
      ),
    );
    assert.equal(injectedTenant.status, 400);

    const registeredResponse = await operator.app.handleRequest(
      request(
        "operator-secret",
        "POST",
        "/v1/global-trust/use-cases",
        useCaseBody,
        confirmation,
      ),
    );
    assert.equal(registeredResponse.status, 201);
    const registered = JSON.parse(registeredResponse.body);
    assert.equal(registered.tenantId, "tenant_001");
    assert.equal(registered.event.descriptor.status, "draft");
    assert.equal(registered.executedUseCase, false);
    assert.equal(registered.modelExecuted, false);
    assert.equal(registered.providerContacted, false);

    const approvedResponse = await operator.app.handleRequest(
      request(
        "operator-secret",
        "POST",
        "/v1/global-trust/use-cases/use_case_support_v1/status",
        {
          status: "approved",
          reasonCode: "governance_review_passed",
          correlationId: "corr_use_case_approve",
        },
        confirmation,
      ),
    );
    assert.equal(approvedResponse.status, 200);
    const approved = JSON.parse(approvedResponse.body);
    assert.equal(approved.result.descriptor.status, "approved");
    assert.equal(approved.executedUseCase, false);
    assert.equal(approved.modelExecuted, false);
    assert.equal(approved.providerContacted, false);

    const listResponse = await operator.app.handleRequest(
      request(
        "operator-secret",
        "GET",
        "/v1/global-trust/use-cases?status=approved",
      ),
    );
    assert.equal(listResponse.status, 200);
    const list = JSON.parse(listResponse.body);
    assert.equal(list.count, 1);
    assert.equal(list.useCases[0].tenantId, "tenant_001");
    assert.equal(list.sensitiveContentIncluded, false);
    assert.equal(
      JSON.stringify(list).includes("operator-secret"),
      false,
    );

    const historyResponse = await operator.app.handleRequest(
      request(
        "operator-secret",
        "GET",
        "/v1/global-trust/use-cases/use_case_support_v1/history",
      ),
    );
    assert.equal(historyResponse.status, 200);
    assert.equal(JSON.parse(historyResponse.body).count, 2);

    const integrityResponse = await operator.app.handleRequest(
      request(
        "operator-secret",
        "GET",
        "/v1/global-trust/use-cases/integrity",
      ),
    );
    assert.equal(integrityResponse.status, 200);
    assert.equal(
      JSON.parse(integrityResponse.body).verification.valid,
      true,
    );

    const executeResponse = await operator.app.handleRequest(
      request(
        "operator-secret",
        "POST",
        "/v1/global-trust/use-cases/use_case_support_v1/execute",
        {},
        confirmation,
      ),
    );
    assert.equal(executeResponse.status, 404);

    const otherTenant = createUseCaseRegisteredOperationalGateway({
      stateFilePath,
      adminKey: "other-secret",
      adminPrincipal: principal(
        "tenant_other",
        "operator_other",
        "human",
        ["usecase:read", "usecase:write"],
      ),
    });
    const otherListResponse = await otherTenant.app.handleRequest(
      request(
        "other-secret",
        "GET",
        "/v1/global-trust/use-cases",
      ),
    );
    assert.equal(otherListResponse.status, 200);
    const otherList = JSON.parse(otherListResponse.body);
    assert.equal(otherList.count, 0);
    assert.deepEqual(otherList.useCases, []);
    assert.equal(
      JSON.stringify(otherList).includes("tenant_001"),
      false,
    );

    const service = createUseCaseRegisteredOperationalGateway({
      stateFilePath,
      adminKey: "service-secret",
      adminPrincipal: principal(
        "tenant_001",
        "service_001",
        "service",
        ["usecase:read", "usecase:write"],
      ),
    });
    const serviceWrite = await service.app.handleRequest(
      request(
        "service-secret",
        "POST",
        "/v1/global-trust/use-cases/use_case_support_v1/status",
        {
          status: "suspended",
          reasonCode: "automated_change",
          correlationId: "corr_service",
        },
        confirmation,
      ),
    );
    assert.equal(serviceWrite.status, 403);
    assert.equal(
      JSON.parse(serviceWrite.body).error,
      "human_operator_required",
    );

    const reader = createUseCaseRegisteredOperationalGateway({
      stateFilePath,
      adminKey: "reader-secret",
      adminPrincipal: principal(
        "tenant_001",
        "reader_001",
        "human",
        ["usecase:read"],
      ),
    });
    const readerWrite = await reader.app.handleRequest(
      request(
        "reader-secret",
        "POST",
        "/v1/global-trust/use-cases/use_case_support_v1/status",
        {
          status: "suspended",
          reasonCode: "missing_scope",
          correlationId: "corr_reader",
        },
        confirmation,
      ),
    );
    assert.equal(readerWrite.status, 403);
    assert.deepEqual(
      JSON.parse(readerWrite.body).authorizationDecision.reasonCodes,
      ["missing_scope:usecase:write"],
    );
  } finally {
    await rm(directory, { recursive: true, force: true });
  }
});
