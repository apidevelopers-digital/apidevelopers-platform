import{authorize}from"@apidevelopers/auth-core";import{createAccessGrantId,createCanonicalId,createEntitlementId,createProvisioningJobId,createSubscriptionId,createTenantId,createWorkspaceId}from"@apidevelopers/contracts";
export const UNI_CO_PROVISIONING_PRODUCT_ID="product:uni-co";
export const MITRA_PROVISIONING_PRODUCT_ID="product:mitra";
const PRODUCT_SLUGS=Object.freeze({[UNI_CO_PROVISIONING_PRODUCT_ID]:"uni-co",[MITRA_PROVISIONING_PRODUCT_ID]:"mitra"});
const DEFAULT_PRODUCT_ID=UNI_CO_PROVISIONING_PRODUCT_ID,PROVIDER="unico-operator-session",SCOPE="saas:provision",PLAN_ID="internal-preview",WEB_SCOPE="web:chat";
const HEX64=/^[a-f0-9]{64}$/,SLUG=/^[a-z0-9]+(?:-[a-z0-9]+)*$/,IDEM=/^[A-Za-z0-9_.:-]{8,200}$/;
...
entitlement_get:"uni_co_customer_membership_lookup_failed",
entitlement_grant:"uni_co_customer_membership_grant_failed",
entitlement_binding:"uni_co_customer_membership_binding_mismatch",
entitlement_status:"uni_co_customer_membership_not_active",
...
const reply=(status,payload)=>Object.freeze({status,headers:Object.freeze({"content-type":"application/json; charset=utf-8","cache-control":"no-store"}),body:JSON.stringify(payload)});
...
function bodyOf(value){if(value&&typeof value==="object"&&!Array.isArray(value))return value;const raw=String(value??"").trim();if(!raw)throw new TypeError("body_required");const parsed=JSON.parse(raw);if(!parsed||typeof parsed!=="object"||Array.isArray(parsed))throw new TypeError("body_invalid");return parsed}
...
function resolveProduct(inputProductId){const productId=String(inputProductId??DEFAULT_PRODUCT_ID).trim()||DEFAULT_PRODUCT_ID;const productSlug=PRODUCT_SLUGS[productId];if(!productSlug)throw new TypeError("productId_invalid");return Object.freeze({productId,productSlug})}
function provisioningReasonForStage(stage){return PROVISIONING_STAGE_REASONS[stage]??"uni_co_provisioning_not_complete"}
...
await saasRuntime.registerTenantWorkspace({tenant:{tenantId,organizationId:createCanonicalId({family:"component",segments:["organization",tenantSlug]}),slug:tenantSlug,displayName,status:"active",createdAt:at},workspace:{workspaceId,tenantId,productId,slug:workspaceSlug,displayName:`${displayName} · ${productSlug}`,status:"active",createdAt:at}});
...
if(job.status==="running")job=await saasRuntime.completeProvisioning({provisioningJobId,at,result:{tenantReady:true,workspaceReady:true,productReady:true,mode:`${productSlug}_internal_preview`}});
...
if(!resolved.resolved){const pending=await saasAccess.grantAccess({accessGrantId,principalId:principal.principalId,tenantId,workspaceId,productId,subscriptionId,entitlementId,requiredScopes:[WEB_SCOPE],grantedScopes:[WEB_SCOPE],status:"pending",createdAt:at});if(pending.status!=="pending")throw new Error("access_grant_not_pending");resolved={resolved:true,grant:await saasAccess.activateAccess({accessGrantId,provisioningJobId,at})}}
...
await saasAccess.setOnboarding({tenantId,workspaceId,productId,status:"completed",requiredSteps:["provisioning_succeeded","access_activated"],completedSteps:["provisioning_succeeded","access_activated"],updatedAt:at});
return reply(201,{ok:true,provisioned:true,tenantId,workspaceId,principalId:principal.principalId,accessGrantId:grant.accessGrantId,productId,status:"active",billing:{mode:"internal-preview",currency:"BRL",monthlyAmount:0},secretsExposed:false});
}catch(error){const message=String(error?.message??"");const invalid=/required|invalid|JSON|idempotency/i.test(message);return reply(invalid?400:409,{ok:false,reason:invalid?"invalid_uni_co_provision_request":provisioningReasonForStage(provisioningStage),secretsExposed:false})}
}});
}
