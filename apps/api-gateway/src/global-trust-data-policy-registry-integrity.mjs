...
export const DATA_POLICY_REGISTRY_EVENT_COLLECTION =
  "global_trust_data_policy_registry_events";
export const DATA_POLICY_REGISTRY_PROOF_COLLECTION =
  "global_trust_data_policy_registry_integrity_proofs";
...
function tenantProofs(tx, tenantId) {
  return tx.list(DATA_POLICY_REGISTRY_PROOF_COLLECTION)
    .map(({ value }) => value)
...
export function createGlobalTrustDataPolicyRegistryIntegrity({
  store,
  ...
  function appendInTransaction(tx, {
    ...
    if (collection !== DATA_POLICY_REGISTRY_EVENT_COLLECTION) {
      throw new TypeError(
        "sourceCollection is not protected by data policy registry integrity",
      );
    }
    ...
    if (chainFailures(proofs).length) {
      throw new TypeError("data policy registry integrity chain is invalid");
    }
    if (proofs.some((proof) =>
      proof.sourceCollection === collection && proof.recordId === id
    )) {
      throw new TypeError("data policy registry integrity proof already exists");
    }

    const proof = {
      contractType: "DataPolicyRegistryIntegrityProof",
      contractVersion: "1.0",
    ...
    tx.put(
      DATA_POLICY_REGISTRY_PROOF_COLLECTION,
      proof.proofId,
      Object.freeze(proof),
      { ifAbsent: true },
    );
  ...
  return Object.freeze({
    ...
    async verifyTenant({ tenantId } = {}) {
      ...
      const transaction = await store.transaction((tx) => {
        ...
        const sourceRecords = tx.list(DATA_POLICY_REGISTRY_EVENT_COLLECTION)
          .map(({ value }) => value)
        ...
        for (const event of sourceRecords) {
          const key = `${DATA_POLICY_REGISTRY_EVENT_COLLECTION}\u0000${event.eventId}`;
        ...
        return Object.freeze({
          contractType: "DataPolicyRegistryIntegrityVerification",
          contractVersion: "1.0",
          tenantId: tenant,
          valid:
            failures.length === 0
