...
function requireText(value, name) {
  ...
  if (!normalized) fail("TRUST_EVALUATION_KEY_ENROLLMENT_INVALID_INPUT", `${name} is required`);
  return normalized;
}
...
function normalizePublicKey(recipientPublicKey) {
  ...
  if (key.asymmetricKeyType !== "rsa") {
    fail("TRUST_EVALUATION_KEY_ENROLLMENT_UNSUPPORTED_PUBLIC_KEY", "recipientPublicKey must be RSA");
  }
  ...
  return Object.freeze({
    fingerprint: createHash("sha256").update(spkiDer).digest("base64url"),
    spkiPem: key.export({ type: "spki", format: "pem" }).toString(),
  });
}
function normalizeApproval(approval, organizationId) {
  if (!approval || typeof approval !== "object") {
    fail("TRUST_EVALUATION_KEY_ENROLLMENT_APPROVAL_REQUIRED", "institutionalApproval is required");
  }
  const decision = requireText(approval.decision, "institutionalApproval.decision");
  if (decision !== APPROVAL_DECISION) {
    fail("TRUST_EVALUATION_KEY_ENROLLMENT_NOT_APPROVED", "institutionalApproval.decision must be approved");
  }
...
function validatePersistedProof(record, { organizationId, fingerprint, challengeId }) {
  if (!record) {
    fail("TRUST_EVALUATION_KEY_ENROLLMENT_PROOF_NOT_FOUND", "recipient key proof challenge was not found");
  }
  ...
  if (record.version !== EXPECTED_PROOF_VERSION || record.algorithm !== EXPECTED_PROOF_ALGORITHM) {
    fail(
...
export function createGlobalTrustEvaluationRecipientKeyEnrollmentService({
  ...
  if (typeof clock !== "function") {
    fail("TRUST_EVALUATION_KEY_ENROLLMENT_INVALID_CLOCK", "clock must be a function");
  }
  ...
  return Object.freeze({
    async recordApprovedEnrollment({
      ...
      const proofRecord = state.collections?.[KEY_PROOF_COLLECTION]?.[challengeId] ?? null;
      ...
      const committed = await store.transaction((tx) => {
        ...
        if (current) {
          const same =
            ...
            current.institutionalApproval?.reference === proposed.institutionalApproval.reference;
          if (same) return Object.freeze({ created: false, record: current });
          fail(
    ...
    async getApprovedEnrollment({
      ...
      const enrollmentId = enrollmentIdFor(organizationId);
      const state = await store.read();
      const record = state.collections?.[ENROLLMENT_COLLECTION]?.[enrollmentId] ?? null;
