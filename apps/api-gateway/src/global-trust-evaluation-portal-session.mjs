...
function ttl(value) {
  ...
  if (!Number.isSafeInteger(normalized) || normalized < 60_000 || normalized > 3_600_000) {
    fail("TRUST_EVALUATION_PORTAL_SESSION_INVALID_TTL", "ttlMs must be 60000..3600000");
  }
  return normalized;
}
...
function requireEnrollment(record, organizationId) {
  if (
    !record ||
...
export function createGlobalTrustEvaluationPortalSessionService({
  ...
  const configuredTtlMs = ttl(sessionTtlMs);
  ...
  async function approvedEnrollment(organizationId) {
    ...
    return requireEnrollment(record, normalizedOrganizationId);
  }
  ...
  return Object.freeze({
    ...
    async complete({ organizationId, challengeId, signatureB64u } = {}) {
      ...
      const expiresAt = new Date(Date.parse(issuedAt) + configuredTtlMs).toISOString();
    ...
    async authenticate({ token } = {}) {
      ...
      const state = await store.read();
      const record = state.collections?.[SESSION_COLLECTION]?.[parsed.sessionId] ?? null;
      if (
        !record ||
      ...
      await approvedEnrollment(record.organizationId);
      return Object.freeze({
