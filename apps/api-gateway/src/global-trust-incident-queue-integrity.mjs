...
export const INCIDENT_QUEUE_INTEGRITY_COLLECTION =
  "global_trust_incident_queue_integrity_proofs";
export const INCIDENT_QUEUE_PROTECTED_COLLECTIONS = Object.freeze([
  "global_trust_incidents",
  "global_trust_incident_events",
]);

const SOURCE_SET = new Set(INCIDENT_QUEUE_PROTECTED_COLLECTIONS);
...
function tenantProofs(tx, tenantId) {
  return tx.list(INCIDENT_QUEUE_INTEGRITY_COLLECTION)
    .map(({ value }) => value)
...
export function createGlobalTrustIncidentQueueIntegrity({
  store,
  ...
  function appendInTransaction(tx, {
    ...
    if (!SOURCE_SET.has(collection)) {
      throw new TypeError("sourceCollection is not protected by incident queue integrity");
    }
    ...
    if (chainFailures(proofs).length) {
      throw new TypeError("incident queue integrity chain is invalid");
    }
    if (proofs.some((proof) =>
      proof.sourceCollection === collection && proof.recordId === id
    )) {
      throw new TypeError("incident queue integrity proof already exists");
    }

    const proof = {
      contractType: "IncidentQueueIntegrityProof",
      contractVersion: "1.0",
    ...
    tx.put(
      INCIDENT_QUEUE_INTEGRITY_COLLECTION,
      proof.proofId,
      Object.freeze(proof),
      { ifAbsent: true },
    );
  ...
  return Object.freeze({
    ...
    async verifyTenant({ tenantId } = {}) {
      ...
      const transaction = await store.transaction((tx) => {
        ...
        for (const collection of INCIDENT_QUEUE_PROTECTED_COLLECTIONS) {
        ...
        return Object.freeze({
          contractType: "IncidentQueueIntegrityVerification",
          contractVersion: "1.0",
          tenantId: tenant,
          valid:
            failures.length === 0
