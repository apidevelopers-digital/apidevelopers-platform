...
export const PROMPT_DEFENSE_PROOF_COLLECTION =
  "global_trust_prompt_defense_integrity_proofs";
export const PROMPT_DEFENSE_SOURCE_COLLECTION =
...
function tenantProofs(tx, tenantId) {
  return tx.list(PROMPT_DEFENSE_PROOF_COLLECTION)
    .map(({ value }) => value)
...
export function createGlobalTrustPromptDefenseIntegrity({
  store,
  ...
  function appendInTransaction(tx, {
    ...
    if (collection !== PROMPT_DEFENSE_SOURCE_COLLECTION) {
      throw new TypeError(
        "sourceCollection is not protected by prompt defense integrity",
      );
    }
    ...
    if (chainFailures(proofs).length) {
      throw new TypeError("prompt defense integrity chain is invalid");
    }
    if (proofs.some((proof) =>
      proof.sourceCollection === collection && proof.recordId === id
    )) {
      throw new TypeError("prompt defense integrity proof already exists");
    }

    const proof = {
      contractType: "PromptDefenseIntegrityProof",
      contractVersion: "1.0",
    ...
    tx.put(
      PROMPT_DEFENSE_PROOF_COLLECTION,
      proof.proofId,
      Object.freeze(proof),
      { ifAbsent: true },
    );
  ...
  return Object.freeze({
    ...
    async verifyTenant({ tenantId } = {}) {
      ...
      const transaction = await store.transaction((tx) => {
        ...
        const sourceRecords = tx.list(PROMPT_DEFENSE_SOURCE_COLLECTION)
          .map(({ value }) => value)
        ...
        for (const decision of sourceRecords) {
          const key =
            `${PROMPT_DEFENSE_SOURCE_COLLECTION}\u0000${decision.decisionId}`;
          if (!proofKeys.has(key)) {
            failures.push({
              proofId: "",
              code: "source_record_unprotected",
              sourceCollection: PROMPT_DEFENSE_SOURCE_COLLECTION,
        ...
        return Object.freeze({
          contractType: "PromptDefenseIntegrityVerification",
          contractVersion: "1.0",
          tenantId: tenant,
          valid:
            failures.length === 0
