...
export const MODEL_REGISTRY_EVENT_COLLECTION =
  "global_trust_model_registry_events";
export const MODEL_REGISTRY_PROOF_COLLECTION =
  "global_trust_model_registry_integrity_proofs";
...
function tenantProofs(tx, tenantId) {
  return tx.list(MODEL_REGISTRY_PROOF_COLLECTION)
    .map(({ value }) => value)
...
export function createGlobalTrustModelRegistryIntegrity({
  store,
  ...
  function appendInTransaction(tx, {
    ...
    if (collection !== MODEL_REGISTRY_EVENT_COLLECTION) {
      throw new TypeError("sourceCollection is not protected by model registry integrity");
    }
    ...
    if (chainFailures(proofs).length) {
      throw new TypeError("model registry integrity chain is invalid");
    }
    if (proofs.some((proof) =>
      proof.sourceCollection === collection && proof.recordId === id
    )) {
      throw new TypeError("model registry integrity proof already exists");
    }

    const proof = {
      contractType: "ModelRegistryIntegrityProof",
      contractVersion: "1.0",
    ...
    tx.put(
      MODEL_REGISTRY_PROOF_COLLECTION,
      proof.proofId,
      Object.freeze(proof),
      { ifAbsent: true },
    );
  ...
  return Object.freeze({
    ...
    async verifyTenant({ tenantId } = {}) {
      ...
      const transaction = await store.transaction((tx) => {
        ...
        const sourceRecords = tx.list(MODEL_REGISTRY_EVENT_COLLECTION)
          .map(({ value }) => value)
        ...
        for (const event of sourceRecords) {
          const key = `${MODEL_REGISTRY_EVENT_COLLECTION}\u0000${event.eventId}`;
        ...
        return Object.freeze({
          contractType: "ModelRegistryIntegrityVerification",
          contractVersion: "1.0",
          tenantId: tenant,
