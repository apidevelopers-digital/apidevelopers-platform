...
function validateProcessResult(result) {
  if (!result || typeof result !== "object" || Array.isArray(result)) {
    fail(
      "keychain_helper_process_contract_violation",
      "Keychain helper process returned an invalid result",
    );
  }
  if (!(result.stdout instanceof Uint8Array)) {
    fail(
      "keychain_helper_process_contract_violation",
      "Keychain helper stdout must be a Uint8Array",
    );
  }
  if (result.stderr !== undefined && !(result.stderr instanceof Uint8Array)) {
    fail(
      "keychain_helper_process_contract_violation",
      "Keychain helper stderr must be a Uint8Array when provided",
    );
  }
  if (!Number.isSafeInteger(result.exitCode)) {
    fail(
      "keychain_helper_process_contract_violation",
      "Keychain helper exit code must be an integer",
    );
  }
  if (result.timedOut !== undefined && typeof result.timedOut !== "boolean") {
    fail(
      "keychain_helper_process_contract_violation",
      "Keychain helper timeout marker must be boolean",
    );
  }
...
function parseHelperResponse(stdout, maxOutputBytes) {
  if (stdout.byteLength < 1) {
    fail("keychain_helper_empty_output", "Keychain helper returned empty output");
  }
  if (stdout.byteLength > maxOutputBytes) {
    fail(
      "keychain_helper_output_too_large",
      "Keychain helper output exceeded the allowed size",
    );
  }
  ...
  try {
    ...
    fail(
      "keychain_helper_invalid_response",
      "Keychain helper returned an invalid response",
    );
  }
  ...
  if (
    ...
    keys.some((value, index) => value !== expectedKeys[index])
  ) {
    fail(
      "keychain_helper_invalid_response",
      "Keychain helper response shape is not allowlisted",
    );
  }
  ...
  if (
    ...
    fail(
      "keychain_helper_invalid_response",
      "Keychain helper did not confirm a new non-replaced item",
    );
  }
...
export function createOperatorMacosKeychainHelperBridge({
  ...
  return async function operatorMacosKeychainHelperBridge({
    ...
    if (!enabled) {
      fail(
        "keychain_helper_bridge_disabled",
        "macOS Keychain helper bridge is disabled by default",
      );
    }
    if (normalizedPlatform !== "darwin") {
      fail(
        "keychain_helper_bridge_platform_denied",
        "macOS Keychain helper bridge is only available on darwin",
      );
    }
    if (
      ...
      fail(
        "keychain_helper_bridge_descriptor_denied",
        "macOS Keychain helper bridge descriptor is not allowlisted",
      );
    }
    if (overwrite !== false) {
      fail(
        "keychain_helper_bridge_overwrite_denied",
        "macOS Keychain helper bridge overwrite is denied",
      );
    }
    if (returnSecret !== false) {
      fail(
        "keychain_helper_bridge_secret_return_denied",
        "macOS Keychain helper bridge cannot return secret material",
      );
    }
    ...
    if (secretBytes.byteLength < 1 || secretBytes.byteLength > MAX_SECRET_BYTES) {
      fail(
        "keychain_helper_bridge_secret_size_denied",
        "macOS Keychain helper secret size is outside the allowed range",
      );
    }
    ...
    try {
      ...
      try {
        ...
        fail(
          "keychain_helper_process_failed",
          "macOS Keychain helper process failed",
        );
      }
      ...
      if (result.timedOut) {
        fail(
          "keychain_helper_process_timeout",
          "macOS Keychain helper process timed out",
        );
      }
      if (result.exitCode !== 0) {
        fail(
          "keychain_helper_operation_failed",
          "macOS Keychain helper operation failed",
        );
      }
